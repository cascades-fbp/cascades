...
func main() {
	...
	// Monitoring setup
	err = runtime.SetupShutdownByDisconnect(context, receiver, "http-handler.in", ch)
	if err != nil {
	...
	for {
		...
		if err != nil {
			log.Println("Error convering IP to request message:", err.Error())
			continue
		}
		...
		if err != nil {
			log.Println("Error convering response to IP:", err.Error())
			continue
		}
		sender.SendMultipart(ip, 0)
		log.Println("Response IP sent")
	}
}
